...
class ServerApi {
	...
	getApis(callback) {
		...
		for (let api of apis) {
			let apiName = api[0]
			if (apiName === '__metadata__') continue
			let version = this._findLatestVersion(api[1])
			let apiObject = api[1].get(version)
			let exposedApi

			if (isFunction(apiObject)) {
				result[apiName] = []
				continue
			}
			 
			let proto = Object.getPrototypeOf(apiObject)
			
			if (proto === Object.prototype) {
				exposedApi = Object.keys(apiObject)
			} else {
				exposedApi = Object.getOwnPropertyNames(proto)
			}

			result[apiName] = exposedApi
		}
